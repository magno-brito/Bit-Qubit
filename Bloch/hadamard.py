...
# Definir a matriz Hadamard manualmente
H = (1 / np.sqrt(2)) * Qobj([[1, 1], [1, -1]])

# Criar estados intermediários aplicando frações da transformação Hadamard
states = []
steps = 20
for i in range(steps + 1):
    # Calcular o estado intermediário aplicando frações do Hadamard
    intermediate_state = (initial_state + i/steps * (H * initial_state - initial_state)).unit()
    states.append(intermediate_state)

# Animação da transição de |0> para |+>
