...
# Definir a rotação com a porta R_y(θ)
theta = np.pi / 2  # Definindo o valor de θ, por exemplo, pi/2
R_y = Qobj([[np.cos(theta / 2), -np.sin(theta / 2)], 
            [np.sin(theta / 2), np.cos(theta / 2)]])  # Matriz da porta R_y(θ)

# Criar estados intermediários aplicando frações da rotação R_y(θ)
states = []
steps = 40
for i in range(steps + 1):
    # Aplica a rotação da porta R_y(θ) em cada passo
    rotation = Qobj([[np.cos(i * theta / (2 * steps)), -np.sin(i * theta / (2 * steps))], 
                     [np.sin(i * theta / (2 * steps)), np.cos(i * theta / (2 * steps))]])  # R_y incremental
    intermediate_state = (rotation * initial_state).unit()  # Aplica a rotação e normaliza o vetor
    states.append(intermediate_state)

# Animação da rotação completa com a porta R_y(θ)
