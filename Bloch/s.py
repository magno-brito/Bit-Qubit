...
# Definir a rotação com a porta S (equivalente a uma rotação Z de pi/2)
S = Qobj([[1, 0], [0, 1j]])  # Matriz da porta S

# Criar estados intermediários aplicando frações da rotação S
states = []
steps = 40
for i in range(steps + 1):
    # Aplica a rotação da porta S em cada passo
    rotation = (S * (i * np.pi / (2 * steps))).expm()  # Exponenciação da matriz S
    intermediate_state = (rotation * initial_state).unit()  # Normaliza o vetor
    states.append(intermediate_state)

# Animação da rotação completa com a porta S
