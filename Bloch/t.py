...
# Definir a rotação com a porta T (equivalente a uma rotação Z de pi/4)
T = Qobj([[1, 0], [0, 1j]])  # Matriz da porta T

# Criar estados intermediários aplicando frações da rotação T
states = []
steps = 40
for i in range(steps + 1):
    # Aplica a rotação da porta T em cada passo
    rotation = (T * (i * np.pi / (4 * steps))).expm()  # Exponenciação da matriz T
    intermediate_state = (rotation * initial_state).unit()  # Normaliza o vetor
    states.append(intermediate_state)

# Animação da rotação completa com a porta T
